...
/**
 * Seeds the tenant_email_addresses table, mapping tenant IDs and audit fields.
 ...
 */
export async function seedTenantEmailAddresses(
  prisma: PrismaClient,
  tenantIds: number[],
  usernameToId: { [key: string]: number },
  auditUserId: number
): Promise<void> {
  for (const item of tenant_email_addresses) {
  ...
  console.log('Seeded tenant email addresses');
}
